...
void scanLargeArray( unsigned int gridNumElems, cl_mem data_d, cl_context clContext, cl_command_queue clCommandQueue, const cl_device_id clDevice, size_t *workItemSizes) {
    ...
    free(source);
    OCL_ERRCK_RETVAL ( ciErrNum = clBuildProgram(scanLargeArray_program, 1, &clDevice, compileOptions, NULL, NULL) ); 

    /*
    // Uncomment for build log from compiler for debugging
    char *build_log;
    size_t ret_val_size;
    ciErrNum = clGetProgramBuildInfo(scanLargeArray_program, clDevice, CL_PROGRAM_BUILD_LOG, 0, NULL, &ret_val_size);
    OCL_ERRCK_VAR(ciErrNum);
